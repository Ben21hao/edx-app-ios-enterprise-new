...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    contentHeight = _scrollView.contentSize.height; // 获取_scrollView的contentSize

    // 判断是否在拖动_scrollView
    if (_scrollView.dragging) {
        int currentPostion = _scrollView.contentOffset.y;

        if (!isRefresh) { // 判断是否正在刷新  否则不做任何操作
            
            [UIView animateWithDuration:0.3 animations:^{
                
                if (currentPostion < -headerHeight * 1.5) { // 当currentPostion 小于某个值时 变换状态
                    headerLabel.text = _titleRelease;
                    
                } else {
                    int currentPostion = _scrollView.contentOffset.y;
                    
                    if (currentPostion - lastPosition > 5) { // 判断滑动方向 以让“松开以刷新”变回“下拉可刷新”状态
        ...
        if ([headerLabel.text isEqualToString:_titleRelease]) {  // 进入刷新状态
