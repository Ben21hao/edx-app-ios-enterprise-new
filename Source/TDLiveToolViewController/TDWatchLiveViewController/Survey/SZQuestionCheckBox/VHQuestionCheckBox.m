...
@interface VHQuestionCheckBox () <UITableViewDelegate,UITableViewDataSource>
...
-(void)uploadContent {
    ...
    if (self.isComplete) {
        ...
        for (int i = 0 ; i<_survey.questionArray.count; i++) {
            ...
            if (question.isMustSelect && [[resultDic objectForKey:[NSString stringWithFormat:@"%d",i]] isEqualToString:@"0"]) {
                [_waringLabel setText:[NSString stringWithFormat:@"第%d题为必填题，请填写完整在提交!",i+1]];
                    return;
            }
        }
        [self uploadDataMethod];
    ...
    for (int i =0 ; i<self.sourceArray.count; i++) {
        ...
        if ([questionAnswer[@"type"] integerValue] == SZQuestionOpenQuestion) {
            ...
            if (str.length > 0) {
                ...
                [dic setValue:[NSString stringWithFormat:@"%ld",(unsigned long)question.questionId] forKey:@"ques_id"];
