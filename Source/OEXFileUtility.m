...
+ (NSString *)applicationSupportPath { //NSApplicationSupportDirectory 本地应用支持文件目录
...
+ (NSString *)pathForUserName:(NSString *)userName { //拼接新路劲，移除旧文件
...
+ (NSString *)pathForUserNameCreatingIfNecessary:(NSString *)userName {
    ...
    if(![fileManager fileExistsAtPath:userDirectory]) {
        ...
        if(![fileManager createDirectoryAtPath:userDirectory withIntermediateDirectories:YES attributes:nil error:&error]) {
    ...
    //防止文件被备份
    NSError *error = nil;
    if(![[NSURL fileURLWithPath:userDirectory] setResourceValue: @YES forKey: NSURLIsExcludedFromBackupKey error: &error]) {
...
+ (NSString *)filePathForRequestKey:(NSString *)url { //url拼接在username后面
