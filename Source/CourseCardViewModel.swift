...
//        return course.courseImageURL
        guard let URL = course.courseImageURL?.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding) else {
            return ""
        }
        return URL
    }
    ...
    func apply(card : CourseCardView, networkManager: NetworkManager,type: NSInteger) { //type : 1 课程详情，2 发现课程，3 我的课程 ，4 已购买课程详情， 5 我的下载
        ...
        if let relativeImageURL = courseImageURL,imageURL = NSURL(string: relativeImageURL, relativeToURL: networkManager.baseURL)
        {
            remoteImage = RemoteImageImpl( url: imageURL.absoluteString!, networkManager: networkManager, placeholder: placeholder, persist: persistImage)
        } else {
