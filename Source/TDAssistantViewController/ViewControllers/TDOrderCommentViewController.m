...
- (void)handinComment {
    ...
    [manager POST:url parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([code intValue] == 200) {
            ...
            [self.view makeToast:TDLocalizeSelect(@"COMMENT_SUBMITTED", nil) duration:1.08 position:CSToastPositionCenter];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self.navigationController popViewControllerAnimated:YES];
            });
            
        } else if ([code intValue] == 312) {
            [self.view makeToast:TDLocalizeSelect(@"HAVE_COMMNET", nil) duration:1.08 position:CSToastPositionCenter];
        } else if ([code intValue] == 500) {
            [self.view makeToast:TDLocalizeSelect(@"UNABEL_SUBMIT_COMMNET", nil) duration:1.08 position:CSToastPositionCenter];
        } else {
            NSLog(@"提交评论有问题 --- %@",code);
        }
